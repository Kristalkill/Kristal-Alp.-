...
module.exports = class Boxes {
    ...
    example_generator(level){
		  const operator = {
			...
			2: '/',
            3: '%',
            4: `% ${this.rn(3)} ${[Math.random() > 0.5 ? '+' : '-','*','/'][Math.floor(Math.random() * 4)]}`
		  };
	...
	spawnrandombox(message){
		...
		.setDescription(`Напишите ответ **${message.guild.settings.Moderation.prefix}pick ${example}**!`)
		.setColor('GREEN')
		.setTimestamp()).then(message=>{
			const collector = msg.channel.createMessageCollector(m => !m.author.bot, {time: 15000});
			collector.on('collect', msg => {
				if(msg.content.toLowerCase() === `${message.guild.settings.Moderation.prefix}pick ${result}`){
                message.edit(new MessageEmbed()
				.setTitle(`Коробка \`${win}\` собрана!`)
				.setDescription(`Коробка \`${win}\` была собрана участником ${message.author}.`)
				.setColor('BLURPLE')
				.setTimestamp()).then(msg => msg.delete(10000))
				msg.channel.send(`[Коробка]: Коробка \`${win}\` досталась участнику ${message.author}!`);
		...
		setTimeout(() => {
			...
			.setDescription(`Коробка редкостью \`${win}\` не была никем собрана.\nОтвет на пример: **${result}**`)
			.setColor('RED')
			.setTimestamp())
		},15000)
