module.exports = class Queue {
  constructor(player, Main) {
    ...
    this.player
      .on('end', async (evt) => {
        ...
        if (!this.message.guild || !this.message.guild.me.voice.channel)
          return this.end('?');
  ...
  loop(type) {
    if (
      typeof type !== 'string' ||
      !['song', 'queue'].includes(type.toLowerCase())
    )
      return this.loop;

    return this.loop === type.toLowerCase()
      ? (this.loop = undefined)
      : (this.loop = type.toLowerCase());
  }
  ...
  async end(reason) {
    switch (reason) {
      ...
      case 'empty':
        await this.destroy();
        this.message.channel.send('Очередь пустая');
        break;
      case 'emptyVC':
        await this.destroy();
        this.message.channel.send('Канал пустой,я ливаю блять');
        break;
      default:
        await this.destroy();
    }
