...
module.exports = class extends Command {
  ...
  async run(message) {
    ...
    const embed = new MessageEmbed().setTitle('ШАРДЫ');
    const uptime = await this.Main.shard.broadcastEval('this.uptime');
    const ping = await this.Main.shard.broadcastEval(
      'Math.round(this.ws.ping)'
    );
    const emojis = await this.Main.shard.fetchClientValues('emojis.cache.size');
    const channels = await this.Main.shard.fetchClientValues(
      'channels.cache.size'
    );
    const ram = await this.Main.shard.broadcastEval(
    ...
    for (let i = 0; i < this.Main.options.shardCount; i++) {
      embed.addField(
        '** **',
        language.shards.param.translate({
          i: i + 1,
          ...
          memory: this.Main.utils.formatBytes(ram[i]),
          guilds: guilds[i],
          user: users[i],
          channels: channels[i],
          emojis: emojis[i],
