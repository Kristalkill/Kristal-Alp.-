...
module.exports = class extends Command {
      ...
      try {
          ...
          if(!message.guild.roles.cache.get(message.guild.settings.Moderation.muterole)){
            if(muterole){
          ...
          if(res1) return message.channel.send(language.mute.params.param1.translate({member:member,time:humanizeDuration(ms(res1.time - Date.now()),{round: true,language: message.guild.settings.Moderation.language})}));
          ...
          message.channel.send(language.mute.params.param1.translate({member:member,reason:reason,time: time == false ? language.mute.params.param2 : humanizeDuration(ms(args[2]),{round: true,language: message.guild.settings.Moderation.language})}))
        } catch (error) {
          console.log(error)
      }}}