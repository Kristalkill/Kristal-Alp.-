...
module.exports = class extends Command {
  ...
  run(message, args) {
    try {
      ...
      const Data = this.Main.db.Block.findOne({ id: member.id });
      if (Data)
        return message.channel.send(
          this.Main.embeds.Main.ErrEmbed.setDescription(
            language.block.param3.translate({ member, reason: Data.reason })
          )
        );
      this.Main.db.Block.create({ id: member.id, reason });
      message.channel.send(
        this.Main.embeds.OKEmbed.setDescription(
          language.block.param4.translate({ member, reason })
        )
      );
    } catch (error) {
      console.log(error);
